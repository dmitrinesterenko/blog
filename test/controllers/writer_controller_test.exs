defmodule BlogPhoenix.WriterControllerTest do
  ...
  setup do
    {:ok, [title: "title", body: "The winter falls were almost frozen", post_id: "05478f7b-2423-4698-983f-8c111a4fc85b", "new_body": "newtown text"]}
  end
  ...
  test "PUT /write/:post_id saves new content for the post", context do
    ...
    assert html_response(conn, 302) =~ "redirected"
    conn = get build_conn(), "/write/#{context[:post_id]}"
    assert html_response(conn, 200) =~ context[:new_body]
    refute html_response(conn, 200) =~ context[:body]
